...
def index():
    ...
    if request.method == 'POST':
        ...
        if allowed_file(cookie_file.filename) and allowed_file(comment_file.filename):
            ...
            valid_cookies = []
            for cookie in cookies_data:
                response = make_request('https://business.facebook.com/business_locations', headers, {'Cookie': cookie} if isinstance(cookie, str) else cookie)
            ...
            def run_task(task_id, id_post, comments, valid_cookies, delay):
                for comment in comments:
                    ...
                    if not valid_cookies:  # No valid cookies left
                    ...
                    valid_cookies.append((current_cookie, token_eaag))  # Rotate usage
